...
public class EXPT extends FunctionBlock {
    ...
    public EXPT(int localId, int executionOrderId, Map<String, Connector> inputMap, Map<String, Connector> outputMap) {
        super(localId, executionOrderId);
    ...
    public Map<String, Connector> execute()
        ...
        if (getInputs().size() >= 2) {
            ...
            for (int i = 1; i < getInputs().size(); i++) {
                ...
                setOutputValue(OUTPUT_OUT, getOutput(OUTPUT_OUT).expt(in));
            }
        }
