...
public class DIV extends FunctionBlock {
    ...
    public DIV(int localId, int executionOrderId, Map<String, Connector> inputMap, Map<String, Connector> outputMap) {
        super(localId, executionOrderId);
        ...
        try {
            ...
            if (getInputs().size() >= 2) {
                ...
                for (int i = 1; i < getInputs().size(); i++) {
                    ...
                    setOutputValue(OUTPUT_OUT, getOutput(OUTPUT_OUT).div(in));
                }
            }
        } catch (ArithmeticException e) {

            throw new NonSupportedOperationException(e.getMessage());
        }
